...
(varconfig! optimize:sealfns fix-modules)
...
(define (pptrize! module (static #t))
  (when module-fixer 
    (if (symbol? module)
...
(defambda (dotighten expr env bound opts lexrefs w/rails)
  ...
  (cond ((ambiguous? expr)
	 (for-choices (each expr) 
	   (dotighten each env bound opts lexrefs w/rails)))
	((fail? expr) expr)
	((symbol? expr)
	...
	((or (pair? (car expr)) (ambiguous? (car expr)))
	 (tighten-call expr env bound opts lexrefs w/rails))
	...
	(else (let* ((head (get-arg expr 0))
		...
		(cond ((if from 
			 ...
			 (when (and (procedure-arity value)
		       ...
		       (callcons (qc (map-opcode
				      (cond ((not from) (try value head))
					    ...
					    (else value))
				      n-exprs))
				 (tighten-args (cdr expr) env bound opts lexrefs w/rails)
				 w/rails))
		      ((and (ambiguous? value)
			...
			"Inconsistent ND call (mixed special/applicable)")
			 ...
			 (if (exists? tightener)
			     (tightener value expr env bound opts lexrefs w/rails)
			     expr)))
		      ((macro? value)
		       (dotighten (macroexpand value expr) env
				  bound opts lexrefs w/rails))
		      (else
  ...
  (if (pair? expr)
      ((if w/rails ->rail ident)
       (if (or (symbol? (car expr)) (pair? (car expr))
	       (ambiguous? (car expr)))
	   `(,(dotighten (car expr) env bound opts lexrefs w/rails)
	     . ,(if (pair? (cdr expr))
		    (tighten-args (cdr expr) env bound opts lexrefs w/rails)
		    (cdr expr)))
...
(define (optimize-arglist arglist env opts lexrefs w/rails)
  (if (pair? arglist)
      (cons
	   ...
	   `(,(caar arglist) 
	     ,(dotighten (cadr (car arglist)) env '() opts lexrefs w/rails))
	   (car arglist))
...
(define (optimize-module! module (opts) (lexrefs) (w/rails))
  ...
  (default! lexrefs (getopt opts 'lexrefs lexrefs-dflt))
  (default! w/rails (getopt opts 'rails rails-dflt))
  (when (getopt opts 'staticmodules static-modules) 
    (dereference! module))
  (when (getopt opts 'staticmodules static-modules) 
    (pptrize! module (getopt opts 'staticmodules static-modules)))
...
(define (tighten-block handler expr env bound opts lexrefs w/rails)
  (cons (map-opcode handler (length (cdr expr)))
	(map (lambda (x) (dotighten x env bound opts lexrefs w/rails))
	     (cdr expr))))
(define (tighten-block->rail handler expr env bound opts lexrefs w/rails)
  (if w/rails
      (->rail (tighten-block handler expr env bound opts lexrefs w/rails))
      (tighten-block handler expr env bound opts lexrefs w/rails)))

(define (tighten-let handler expr env bound opts lexrefs w/rails)
  (let* ((bindexprs (cadr expr))
	 (new-bindexprs
	  (map (lambda (x)
		 `(,(car x) ,(dotighten (cadr x) env bound opts lexrefs w/rails)))
	       bindexprs))
	 (body (cddr expr)))
    `(,handler ,new-bindexprs
	       ,@(let ((bound (cons (map car bindexprs) bound)))
		   (map (lambda (b) (dotighten b env bound opts lexrefs w/rails))
			body)))))
(define (tighten-doexpression handler expr env bound opts lexrefs w/rails)
  (let ((bindspec (cadr expr)) (body (cddr expr)))
    `(,handler (,(car bindspec)
		,(dotighten (cadr bindspec) env bound opts lexrefs w/rails)
		,@(cddr bindspec))
	       ,@(let ((bound
		   ...
		   (map (lambda (b) (dotighten b env bound opts lexrefs w/rails))
			body)))))
(define (tighten-do2expression handler expr env bound opts lexrefs w/rails)
  (let ((bindspec (cadr expr)) (body (cddr expr)))
    `(,handler ,(cond ((pair? bindspec)
			 ...
			 ,@(cddr bindspec)))
		      ((symbol? bindspec)
		       `(,bindspec
			 ,(dotighten bindspec env bound opts lexrefs w/rails)))
...
(define (tighten-dosubsets handler expr env bound opts lexrefs w/rails)
  (let ((bindspec (cadr expr)) (body (cddr expr)))
    `(,handler (,(car bindspec)
		,(dotighten (cadr bindspec) env bound opts lexrefs w/rails)
		,@(cddr bindspec))
...
(define (tighten-let*-bindings bindings env bound opts lexrefs w/rails)
  (if (null? bindings) '()
      `((,(car (car bindings))
	 ,(dotighten (cadr (car bindings)) env bound opts lexrefs w/rails))
	,@(tighten-let*-bindings
	   (cdr bindings) env
	   (cons (append (car bound) (list (car (car bindings))))
		 (cdr bound))
	   opts lexrefs w/rails))))

(define (tighten-let* handler expr env bound opts lexrefs w/rails)
  (let ((bindspec (cadr expr)) (body (cddr expr)))
    `(,handler
      ,(tighten-let*-bindings (cadr expr) env (cons '() bound) opts lexrefs w/rails)
      ,@(let ((bound (cons (map car bindspec) bound)))
	  (map (lambda (b) (dotighten b env bound opts lexrefs w/rails))
	       body)))))
(define (tighten-set-form handler expr env bound opts lexrefs w/rails)
  `(,handler ,(cadr expr) ,(dotighten (third expr) env bound opts lexrefs w/rails)))

(define (tighten-lambda handler expr env bound opts lexrefs w/rails)
  `(,handler ,(cadr expr)
...
(define (tighten-unwind-protect handler expr env bound opts lexrefs w/rails)
  `(,handler ,(dotighten (cadr expr) env bound opts lexrefs w/rails)
	     ,@(map (lambda (uwclause)
		      (dotighten uwclause env bound opts lexrefs w/rails))
		    (cddr expr))))

(define (tighten-quasiquote handler expr env bound opts lexrefs w/rails)
  `(,handler ,(tighten-quasiquote-node (cadr expr) env bound opts lexrefs w/rails)))
(defambda (tighten-quasiquote-node expr env bound opts lexrefs w/rails)
  (cond ((ambiguous? expr)
	 (for-choices (elt expr)
	   (tighten-quasiquote-node elt env bound opts lexrefs w/rails)))
	((and (pair? expr) 
	      (or (eq? (car expr) 'unquote)  (eq? (car expr) 'unquote*)))
	 `(,(car expr) ,(dotighten (cadr expr) env bound opts lexrefs w/rails)))
	((pair? expr)
	 (forseq (elt expr)
	   (tighten-quasiquote-node elt env bound opts lexrefs w/rails)))
	((vector? expr)
	...
	((slotmap? expr)
	 (let ((copy (frame-create #f))
	   ...
	   (do-choices (slot slots)
	     (store! copy (tighten-quasiquote-node slot env bound opts lexrefs w/rails)
		     (try (tighten-quasiquote-node (get expr slot) env bound opts lexrefs w/rails)
			  (get expr slot))))
	   copy))
	(else expr)))

(define (tighten-logmsg handler expr env bound opts lexrefs w/rails)
  (if (or (symbol? (cadr expr)) (number? (cadr expr)))
      (if (or (symbol? (caddr expr)) (number? (caddr expr)))
	  `(,handler ,(cadr expr)
		     ,(caddr expr)
		     ,@(map (lambda (elt)
			      (dotighten elt env bound opts lexrefs w/rails))
			    (cdddr expr)))
	  `(,handler ,(cadr expr)
		     ,@(map (lambda (elt)
			      (dotighten elt env bound opts lexrefs w/rails))
...
(define (tighten-logif handler expr env bound opts lexrefs w/rails)
  (if (or (symbol? (caddr expr))  (number? (caddr expr)))
      `(,handler ,(dotighten (cadr expr) env bound opts lexrefs w/rails)
		 ,(caddr expr)
		 ,@(map (lambda (elt)
...
(define (tighten-attribs attribs env bound opts lexrefs w/rails)
  (cond ((not (pair? attribs)) attribs)
	((pair? (cdr attribs))
	 `(,(if (and (pair? (car attribs))
		     (not (eq? (car (car attribs)) 'quote)))
		`(,(car (car attribs))
		  ,(dotighten (cadr (car attribs)) env bound opts lexrefs w/rails))
		(car attribs))
	   ,(if (and (pair? (car attribs))
		     (not (eq? (car (car attribs)) 'quote)))
		(if (pair? (cadr attribs))
		    `(,(car (cadr attribs))
		      ,(dotighten (cadr (cadr attribs)) env bound opts lexrefs w/rails))
		    (dotighten (cadr attribs) env bound opts lexrefs w/rails))
		(dotighten (cadr attribs) env bound opts lexrefs w/rails))
	   ,@(tighten-attribs (cddr attribs) env bound opts lexrefs w/rails)))
	((pair? (car attribs))
	 `((,(car (car attribs))
	    ,(dotighten (cadr (car attribs)) env bound opts lexrefs w/rails))))
	(else attribs)))

(define (tighten-markup handler expr env bound opts lexrefs w/rails)
  `(,(car expr)
    ,@(map (lambda (x) (dotighten x env bound opts lexrefs w/rails))
	   (cdr expr))))
(define (tighten-markup* handler expr env bound opts lexrefs w/rails)
  `(,(car expr) ,(tighten-attribs (second expr) env bound opts lexrefs w/rails)
    ,@(map (lambda (x) (dotighten x env bound opts lexrefs w/rails))
	   (cddr expr))))

(define (tighten-emptymarkup fcn expr env bound opts lexrefs w/rails)
  `(,(car expr) ,@(tighten-attribs (cdr expr) env bound opts lexrefs w/rails)))

(define (tighten-anchor* fcn expr env bound opts lexrefs w/rails)
  `(,(car expr) ,(dotighten (cadr expr) env bound opts lexrefs w/rails)
    ,(tighten-attribs (third expr) env bound opts lexrefs w/rails)
    ,@(map (lambda (elt) (dotighten elt env bound opts lexrefs w/rails))
	   (cdr (cddr expr)))))

(define (tighten-xmlblock fcn expr env bound opts lexrefs w/rails)
  `(,(car expr) ,(cadr expr)
    ,(tighten-attribs (third expr) env bound opts lexrefs w/rails)
    ,@(map (lambda (elt) (dotighten elt env bound opts lexrefs w/rails))
	   (cdr (cddr expr)))))
